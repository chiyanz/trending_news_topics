...
val df_raw = spark.read.option("header", "true").csv("final_project/trending_news_topics/data_ingest/cnbc_news_datase.csv")
val df_withNa = df_raw.select("published_at", "keywords")

val df2_raw = spark.read.option("header", "true").option("quote", "\"").option("escape", "\"").csv("final_project/trending_news_topics/data_ingest/huffpost_news_data.csv")
...
val df2_parsedDate = df2.withColumn("published_at", to_date($"published_at", "MM/dd/yy"))
df2_parsedDate.show()
...
// Add a source column
val df_final = df_cleaned.withColumn("source", lit("CNBC"))
val df2_final = df2_cleaned.withColumn("source", lit("HuffPost"))


// combine 
val df_combined = df_final.union(df2_final)
